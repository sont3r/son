...
def seed_database():
    with app.app_context():
        ...
        required_tables = ['users', 'subject_areas', 'buildings', 'open_days', 'events', 'courses']
        missing_tables = [table for table in required_tables if table not in existing_tables]
        
        if missing_tables:
            print(f"Error: The following required tables are missing: {missing_tables}")
            print("Please run init_db.py first to create all tables.")
            return
        
        print("All required tables exist. Proceeding with data seeding...")
        ...
        db.session.commit()
        print("Basic data committed to database")
